#! /usr/bin/env python
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# This function implements a X vs dX/dt parameter space glitch study
...
def deriv_map(ifo,channel_name,start_time,end_time,sample_rate,frame_choice,snr_low,snr_high,freq_low,freq_high,filter_rate,science_flag,code_version):

    observatory = ifo[0]

    clock_big = time.time()

    command_string = 'python MCIV_start.py -i ' + ifo + ' -t deriv -s ' + start_time + ' -e ' + end_time + ' -x ' + channel_name + \
        ' -f ' + frame_choice + ' -r ' + sample_rate  + \
    ...
    # build channel list for getChannelVector
    ...
    file_prefix = ifo + '_' + channel_name + '_' + start_date_string + '_' + end_date_string + '_'
    ...
    logfile.write('Multi-channel glitch study: X vs dX/dt\n')
    ...
    logfile.write('Derivative glitch map produced for ' + channel + '.\n\n')
    ...
    # Find channel range; this determines the endpoints for the plots and histogramming
    # Channel 1 is vertical
    xmin, xmax = channelRange(ifo,segments,channel_name,end_buffer,segment_min_length)

    clock2 = time.time()
    elapsed = clock2-clock1
    logfile.write('...done.  Elapsed time is ' + str("{0:.2f}".format(elapsed)) + ' seconds.\n\n')

    logfile.write('Range for ' + channel + ' is from ' + str(xmin) + ' to ' + str(xmax) + '\n')

    xspan = xmax - xmin
    xmean = (xmax + xmin)/2.0

    logfile.write('Middle of ' + channel + ' range is ' + str(xmean) + '.  Normalization is ' + str(xspan) + '.\n')
    ...
    segmentMedianValues = array([[ xmean,  0.0]])
    ...
    for segment in segments:
        ...
        for seg in segmentEndPoints:
            ...
            #x = ( vector[:,0] - xmean ) / xspan

            # Filter the data, take the derivative
    ...
    channelX_label = channel
    channelY_label = 'd(' + channel + ')/dt'

    channels = [channelX_label, channelY_label]
    ...
    map_segments_dset = f.create_dataset('map_segments', data=segments)
    ...
    scan_dset = f.create_dataset('scan_type', data='deriv')
    ...
    pylab.xlabel(channelX_label,fontsize=14)
    pylab.ylabel(channelY_label,fontsize=14)
    ...
    pylab.xlabel(channelX_label,fontsize=10)
    pylab.ylabel(channelY_label,fontsize=10)
    ax.set_zlabel('log[seconds]',fontsize=10)
    pylab.title('Time map, ' + channelX_label + ' vs ' + channelY_label,fontsize=12)
    ...
    pylab.title('Glitch Count, ' + channelX_label + ' vs ' + channelY_label,fontsize=12)
    ...
    ax.set_zlim3d(0,40)
    pylab.title('Glitch Rate, ' + channelX_label + ' vs ' + channelY_label,fontsize=12)
